class Run
  ...
  def self.perform
    ...
    puts "Doing job"
      ...
      if schedule.repeat_daily == true || schedule.send("#{Time.now.strftime("%A")}".to_sym.downcase) == true || schedule.remind_over != nil
        bot = schedule.bot
        ...
        if schedule.remind_over != nil
          @bot_action.update_attribute(:remind_over, schedule.remind_over)
        end
